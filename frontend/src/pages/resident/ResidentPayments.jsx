...
function PaymentPanel({ payment, onDone, onCancel }) {
  ...
  if (loading) return <div className="mt-3 bg-slate-50 rounded-xl p-4 text-sm text-slate-400">Loading payment options...</div>;
    ...
    <div className="mt-3 bg-slate-50 rounded-xl p-4">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {options.methods.razorpay && (
              <button onClick={() => setMethod("razorpay")} className="w-full text-left border border-gray-200 rounded-xl p-3 hover:border-brand-400 transition bg-white">
                <p className="font-medium text-sm text-slate-800">Smart PG (Online)</p>
                <p className="text-xs text-slate-400">+ ₹{options.convenienceFee} convenience fee · Auto-verified instantly</p>
              </button>
            )}
            {options.methods.upi && (
              <button onClick={() => setMethod("upi")} className="w-full text-left border border-gray-200 rounded-xl p-3 hover:border-brand-400 transition bg-white">
                <p className="font-medium text-sm text-slate-800">Direct UPI</p>
                <p className="text-xs text-slate-400">Pay owner directly, then upload proof for approval</p>
              </button>
            )}
            {options.methods.cash && (
              <button onClick={() => setMethod("cash")} className="w-full text-left border border-gray-200 rounded-xl p-3 hover:border-brand-400 transition bg-white">
                <p className="font-medium text-sm text-slate-800">Cash</p>
      ...
      {method === "razorpay" && (
        <div className="space-y-3">
          ...
          <p className="text-sm text-slate-600">Total: <strong>₹{(payment.amount + options.convenienceFee).toLocaleString()}</strong> (₹{payment.amount.toLocaleString()} rent + ₹{options.convenienceFee} fee)</p>
        ...
        <div className="space-y-3">
          ...
          {options.upi ? (
            <div className="bg-white rounded-xl p-4 text-center">
              ...
              <p className="font-semibold text-slate-800 mb-2">{options.upi.ownerName}</p>
              <img src={options.upi.qrDataUrl} alt="UPI QR" className="w-40 h-40 mx-auto rounded-xl border border-gray-100 mb-2" />
              <p className="text-xs text-slate-400">UPI ID</p>
              <p className="font-mono text-sm text-slate-700 mb-2">{options.upi.upiId}</p>
              <button type="button" onClick={() => { navigator.clipboard.writeText(options.upi.upiId); toast.success("UPI ID copied"); }} className="btn-secondary text-xs inline-flex items-center gap-1"><Copy size={12} /> Copy UPI ID</button>
            </div>
          ) : (
    ...
    <div>
      ...
      {payments.length === 0 ? (
        ...
        <>
          {due.length > 0 && (
            <div className="mb-8">
              <h2 className="font-heading font-semibold text-amber-700 mb-3">Due ({due.length})</h2>
              <div className="space-y-3">
                {due.map((p) => (
                  <div key={p._id} className="card p-5 border-amber-200 bg-amber-50">
                    <div className="flex justify-between items-center flex-wrap gap-4">
                      <div>
                        ...
                        <p className="amount font-heading text-2xl font-bold text-amber-700 mt-1">₹{p.amount.toLocaleString()}</p>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b">
                      ...
                      <th className="text-left px-4 py-3 font-semibold text-slate-600">PG</th>
                  ...
                  <tbody>
                    {paid.map((p) => (
                      <tr key={p._id} className="border-b hover:bg-slate-50 transition">
                        ...
                        <td className="px-4 py-3 text-slate-500">{p.pg?.name || "—"}</td>
                        <td className="px-4 py-3 text-slate-500">{p.room?.roomNumber}</td>
                        <td className="px-4 py-3 text-slate-500 capitalize">{p.type === "deposit" ? "Deposit" : "Rent"}</td>
                        <td className="amount px-4 py-3 font-semibold text-slate-800">₹{p.amount.toLocaleString()}</td>
                        <td className="px-4 py-3 text-slate-500 capitalize">{p.paymentMethod || "—"}</td>
                        <td className="px-4 py-3 text-slate-500">{p.verifiedBy === "automatic" ? "Automatic" : "By Owner"}</td>
                        <td className="px-4 py-3 text-slate-400">{p.paidAt ? new Date(p.paidAt).toLocaleDateString() : "—"}</td>
                        <td className="px-4 py-3">
                          <button onClick={() => handleDownload(p._id)} className="text-brand-500 hover:underline text-xs inline-flex items-center gap-1"><FileText size={12} /> Download</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
