    ...
    return (
      <div>
        ...
        <div className="card p-10 text-center bg-amber-50 border-amber-200">
          <Home size={36} className="mx-auto text-amber-400 mb-3" strokeWidth={1.5} />
          <p className="font-heading font-semibold text-amber-800">No room assigned yet</p>
          <p className="text-amber-600 text-sm mt-1">Your PG owner will assign you to a room, or scan a QR invite. Check back later!</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="card p-5 mb-4">
        ...
        <p className="text-slate-500 text-sm mt-0.5 flex items-center gap-1"><Home size={13} /> {pg?.address}</p>
        {pg?.contactPhone && <a href={`tel:${pg.contactPhone}`} className="inline-flex items-center gap-1.5 text-brand-500 text-sm mt-1 hover:underline"><Phone size={13} /> {pg.contactPhone}</a>}
        ...
        {pg?.rules && (
          <div className="mt-3 bg-amber-50 rounded-xl p-3 text-sm text-amber-700 flex items-start gap-2">
            <ClipboardList size={16} className="shrink-0 mt-0.5" /> {pg.rules}
          </div>
        )}
      </div>
      ...
      <div className="card p-5 mb-4">
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="text-right">
            <p className="amount font-heading font-bold text-brand-500 text-xl">₹{room.rent?.toLocaleString()}</p>
              ...
              return (
                <div key={r._id} className={`flex items-center gap-3 p-3 rounded-xl ${isMe ? "bg-brand-50 border border-brand-200" : "bg-slate-50"}`}>
                  {r.photoUrl
                    ...
                    : <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center font-bold text-slate-500 shrink-0">{r.name?.charAt(0)}</div>
                  }
      ...
      <div className="card p-5 mb-4">
        ...
        {vacateNotice?.requested ? (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            ...
            <p className="text-amber-600 text-xs mt-1">Your owner has been notified and will process this closer to the date.</p>
