...
const FEATURES = [
  ...
  { icon: LineChart, title: "Revenue analytics", desc: "Trend charts plus downloadable PDF/Excel reports." },
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-paper dark:bg-ink-950">
      {/* ---------------- Hero ---------------- */}
      <section className="border-b border-[#DAD4C4] dark:border-ink-700">
        <div className="max-w-6xl mx-auto px-6 py-20 grid lg:grid-cols-2 gap-14 items-center">
          <div>
            ...
            <h1 className="font-heading text-5xl md:text-6xl font-semibold text-ink-900 dark:text-paper leading-[1.05] mb-6">
              Every rent, receipt<br />and resident —<br />in one book.
            </h1>
            <p className="text-ink-400 dark:text-ink-100 text-lg mb-8 max-w-md leading-relaxed">
            ...
            <div className="flex flex-wrap gap-x-8 gap-y-2 text-sm text-ink-400 dark:text-ink-100">
          ...
          {/* Signature element: a rent receipt rendered like a real ledger
              stub, using the mono "amount" treatment and a perforated
              stub-edge — this is the one piece of visual flourish on the
              page, everything else stays quiet. */}
          <div className="relative mx-auto w-full max-w-sm">
            <div className="card p-0 overflow-hidden rotate-1">
              <div className="bg-ink-900 dark:bg-ink-800 px-6 py-4 flex items-center justify-between">
              ...
              <div className="p-6 stub-edge">
                ...
                <p className="text-ink-900 dark:text-paper font-heading text-lg font-semibold mb-4">
                  Room 204, Sunrise PG
                </p>
                ...
                <div className="border-t border-[#DAD4C4] dark:border-ink-700 pt-4 flex justify-between items-baseline">
                  <span className="text-sm font-semibold text-ink-900 dark:text-paper">Total paid</span>
                  <span className="amount text-2xl font-semibold text-ink-900 dark:text-paper">₹9,520</span>
                </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 grid md:grid-cols-2 gap-8">
        <div className="card p-8">
          ...
          <h2 className="font-heading text-2xl font-semibold text-ink-900 dark:text-paper mb-5">
            Run the property, not the paperwork
          </h2>
          <ol className="space-y-4 mb-7">
            {OWNER_TRACK.map((step, i) => (
              <li key={step} className="flex gap-3 text-sm text-ink-400 dark:text-ink-100">
      ...
      {/* ---------------- Feature grid ---------------- */}
      <section className="bg-ink-900 dark:bg-ink-950 py-20">
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 text-center">
        <h2 className="font-heading text-3xl font-semibold text-ink-900 dark:text-paper mb-3">
          Ready to close the notebook?
        </h2>
        <p className="text-ink-400 dark:text-ink-100 mb-8">Free for owners and residents alike.</p>
      ...
      <footer className="border-t border-[#DAD4C4] dark:border-ink-700 py-8 text-center text-ink-400 text-sm">
        <p>© {new Date().getFullYear()} Smart PG. All rights reserved.</p>
        <div className="flex justify-center gap-6 mt-3">
          <Link to="/pgs" className="hover:text-ink-900 dark:hover:text-paper transition">Browse PGs</Link>
          <Link to="/signup" className="hover:text-ink-900 dark:hover:text-paper transition">Sign Up</Link>
          <Link to="/login" className="hover:text-ink-900 dark:hover:text-paper transition">Login</Link>
        </div>
      </footer>
    </div>
  );
