...
export default function ResidentProfile() {
  ...
  useEffect(() => {
    console.log("[RESIDENT PROFILE] Loading:", residentId);
    api.get(`/rooms/resident/${residentId}/profile`)
      .then(({ data }) => { setResident(data); setLoading(false); })
      .catch((err) => { setError(err.response?.data?.message || "Failed to load profile"); setLoading(false); });
  }, [residentId]);
      ...
      <div className="card p-6 mb-5">
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1">
            ...
            {resident.phone && <p className="text-slate-500 text-sm">📞 {resident.phone}</p>}
          </div>
        </div>
      </div>
        ...
        {resident.idProofType ? (
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              {resident.idProofNumber && <span className="text-sm text-slate-600 font-mono">{resident.idProofNumber}</span>}
            </div>
            {resident.idProofUrl ? (
              <div>
