...
export default function VacatedResidents() {
  ...
  return (
    <div>
      <h1 className="font-heading text-3xl font-bold text-slate-900 mb-2">Vacated Residents</h1>
      <p className="text-slate-500 mb-6 text-sm">Residents who've moved out — their details are kept for reference.</p>
            ...
            <div key={r._id} className="card p-5">
              <div className="flex items-start justify-between gap-3 mb-3">
                <Link to={`/owner/resident/${r._id}`} className="flex items-center gap-3 min-w-0 group">
                  {r.photoUrl
                    ...
                    : <div className="w-11 h-11 rounded-full bg-slate-200 flex items-center justify-center font-bold text-slate-500 shrink-0">{r.name?.charAt(0)}</div>}
                  <div className="min-w-0">
                    <p className="font-semibold text-sm text-slate-800 group-hover:text-brand-500 transition truncate">{r.name}</p>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs border-t border-gray-100 pt-3">
                <div>
                  ...
                  <p className="text-slate-700">{r.emergencyContact || "—"} {r.emergencyPhone ? `(${r.emergencyPhone})` : ""}</p>
                </div>
                <div>
                  <p className="text-slate-400 uppercase tracking-wide font-semibold mb-0.5">ID Proof</p>
                  <p className="text-slate-700">{r.idProofType ? `${r.idProofType} · ${r.idProofNumber || "—"}` : "Not provided"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-slate-700">{r.occupation || r.company || r.college || "—"}</p>
                </div>
              </div>
