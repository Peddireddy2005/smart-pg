      ...
      <form onSubmit={submit} className="card p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input className="input" type="number" required min={form.occupancy || 1}
              value={form.capacity} onChange={(e) => setForm({ ...form, capacity: e.target.value })} />
            {form.occupancy > 0 && (
              <p className="text-xs text-slate-400 mt-1">Can't go below {form.occupancy} (current occupancy)</p>
            )}
          </div>
          <div>
