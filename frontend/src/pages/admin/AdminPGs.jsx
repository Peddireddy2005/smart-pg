...
export default function AdminPGs() {
  ...
  return (
    <div>
      <h1 className="font-heading text-3xl font-bold text-slate-900 mb-6">All PGs</h1>
      {loading ? <p className="text-slate-400">Loading...</p> : (
        <div className="grid md:grid-cols-2 gap-4">
          {pgs.map((pg) => (
            <div key={pg._id} className="card p-5">
              <p className="font-heading font-bold text-slate-900">{pg.name}</p>
              <p className="text-sm text-slate-500 flex items-center gap-1"><MapPin size={13} /> {pg.city}</p>
